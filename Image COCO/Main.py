...
def target_loss(sess, target_lstm, data_loader):
    ...
    for it in xrange(data_loader.num_batch):
        ...
        g_loss, entropy = sess.run(
            [target_lstm.pretrain_loss, target_lstm.cross_entropy],
            {target_lstm.x: batch})
        nll.append(g_loss)
...
def get_reward(model, dis, sess, input_x, rollout_num, dis_dropout_keep_prob,
    ...
    for i in range(rollout_num):
        ...
        for given_num in range(1, model.sequence_length / model.step_size):
            ...
            feed = {
                model.x: input_x,
                model.given_num: real_given_num,
                model.drop_out: 1.0
            }
            ...
            # print samples.shape
            feed = {
                dis.D_input_x: samples,
                dis.dropout_keep_prob: dis_dropout_keep_prob
            }
