...
class LeakGAN(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('Worker'):
            ...
            self.W_workerOut_change = tf.Variable(
                tf.random_normal([self.vocab_size, self.goal_size], stddev=0.1))

            self.g_change = tf.Variable(
                tf.random_normal(
                    [self.goal_out_size, self.goal_size], stddev=0.1))
        ...
        goal = tensor_array_ops.TensorArray(
            dtype=tf.float32,
            size=self.sequence_length,
            dynamic_size=False,
            infer_shape=True,
        ...
        _, _, _, _, self.gen_o, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array = control_flow_ops.while_loop(
            cond=
            lambda i, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11: i < self.sequence_length,
            body=_g_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings,
                                              self.start_token), self.h0_worker,
                       self.h0_manager, gen_o, gen_x, goal,
        ...
        self.gen_x = tf.transpose(
            self.gen_x, perm=[1, 0])  # batch_size x seq_length
        ...
        sub_feature = tensor_array_ops.TensorArray(
            dtype=tf.float32,
            size=self.sequence_length // self.step_size,
            dynamic_size=False,
            infer_shape=True,
        ...
        all_sub_features = tensor_array_ops.TensorArray(
            ...
            infer_shape=True,
            clear_after_read=False)
        all_sub_goals = tensor_array_ops.TensorArray(
            dtype=tf.float32,
            size=self.sequence_length,
            dynamic_size=False,
            infer_shape=True,
        ...
        _, _, self.g_predictions, _, _, _, _, _, self.feature_array,
        self.real_goal_array, self.sub_feature, self.all_sub_features,
        self.all_sub_goals = control_flow_ops.while_loop(
            cond=
            lambda i, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12: i < self.sequence_length + 1,
            body=preTrain,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings,
        ...
        self.real_goal_array = tf.transpose(
            self.real_goal_array, perm=[1, 0, 2])
        ...
        self.pretrain_worker_loss = -tf.reduce_sum(
            tf.one_hot(
                tf.to_int32(tf.reshape(self.x, [-1])), self.vocab_size, 1.0,
                0.0) * tf.log(
                    tf.clip_by_value(
        ...
        self.all_sub_features = tf.transpose(
            self.all_sub_features, perm=[1, 0, 2])
    ...
    def rollout(self, input_x, given_num):
        ...
        #Next is rollout
        gen_for_reward = tensor_array_ops.TensorArray(
            dtype=tf.int32,
            size=1,
            dynamic_size=True,
            infer_shape=True,
        ...
        i, x_t, _, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward, given_num = control_flow_ops.while_loop(
            cond=
            lambda i, _1, _2, _3, _4, _5, _6, _7, given_num: i < given_num + 1,
        ...
        gen_for_reward = tf.transpose(
            gen_for_reward, perm=[1, 0])  # batch_size x seq_length
    ...
    def pretrain_step(self, sess, x, dropout_keep_prob):
        outputs = sess.run(
            [
                self.pretrain_worker_updates, self.pretrain_worker_loss,
                self.pretrain_manager_updates, self.pretrain_goal_loss
            ],
            feed_dict={
                self.x: x,
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.variable_scope('Worker'):
            ...
            self.Wi_worker = tf.Variable(
                tf.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Ui_worker = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi_worker = tf.Variable(
                tf.random_normal([self.hidden_dim], stddev=0.1))

            self.Wf_worker = tf.Variable(
                tf.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uf_worker = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bf_worker = tf.Variable(
                tf.random_normal([self.hidden_dim], stddev=0.1))

            self.Wog_worker = tf.Variable(
                tf.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uog_worker = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bog_worker = tf.Variable(
                tf.random_normal([self.hidden_dim], stddev=0.1))

            self.Wc_worker = tf.Variable(
                tf.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uc_worker = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bc_worker = tf.Variable(
                tf.random_normal([self.hidden_dim], stddev=0.1))
            params.extend([
                self.Wi_worker, self.Ui_worker, self.bi_worker, self.Wf_worker,
                self.Uf_worker, self.bf_worker, self.Wog_worker,
                self.Uog_worker, self.bog_worker, self.Wc_worker,
                self.Uc_worker, self.bc_worker
            ])
            ...
            def unit(x, hidden_memory_tm1):
                ...
                i = tf.sigmoid(
                    ...
                    tf.matmul(previous_hidden_state, self.Ui_worker) +
                    self.bi_worker)

                # Forget Gate
                f = tf.sigmoid(
                    tf.matmul(x, self.Wf_worker) +
                    tf.matmul(previous_hidden_state, self.Uf_worker) +
                    self.bf_worker)

                # Output Gate
                o = tf.sigmoid(
                    tf.matmul(x, self.Wog_worker) +
                    tf.matmul(previous_hidden_state, self.Uog_worker) +
                    self.bog_worker)

                # New Memory Cell
                c_ = tf.nn.tanh(
                    tf.matmul(x, self.Wc_worker) +
                    tf.matmul(previous_hidden_state, self.Uc_worker) +
                    self.bc_worker)
    ...
    def create_Worker_output_unit(self, params):
        with tf.variable_scope('Worker'):
            self.W_worker = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.vocab_size * self.goal_size],
                    stddev=0.1))
            self.b_worker = tf.Variable(
                tf.random_normal(
                    [self.vocab_size * self.goal_size], stddev=0.1))
    ...
    def create_Manager_recurrent_unit(self, params):
        with tf.variable_scope('Manager'):
            ...
            self.Wi = tf.Variable(
                tf.random_normal(
                    [self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Ui = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi = tf.Variable(
            ...
            self.bc = tf.Variable(
                tf.random_normal([self.hidden_dim], stddev=0.1))
            params.extend([
                self.Wi, self.Ui, self.bi, self.Wf, self.Uf, self.bf, self.Wog,
                self.Uog, self.bog, self.Wc, self.Uc, self.bc
            ])
    ...
    def create_Manager_output_unit(self, params):
        with tf.variable_scope('Manager'):
            self.Wo = tf.Variable(
                tf.random_normal(
                    [self.hidden_dim, self.goal_out_size], stddev=0.1))
            self.bo = tf.Variable(
                tf.random_normal([self.goal_out_size], stddev=0.1))
